...
export const PaperLink = ({ to, title = "Download the Paper" }) => {
  return (
    <div>
      <span>
      ...
      <a href={to} className="text-accent hover:text-accent hover:underline" target="_blank" rel="noopener noreferrer">
        {title}
      </a>
    </div>
  )
...
export const NavExtLink = ({ to, children, title = "Link", selected = false }) => {
  const textColor = selected ? "text-accent" : "text-secondary"
  const style = `font-normal text-base hover:text-accent transition duration-150 ${textColor}`
  return (
    <Link title={title} to={to} target="_blank" rel="noopener noreferrer">
      <span className={style}>{children}</span>
    </Link>
  )
}
